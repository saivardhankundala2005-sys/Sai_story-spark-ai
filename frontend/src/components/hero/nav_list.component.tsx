...
const NavListComponent: React.FC = () => {
  ...
  const getLinkClass = (isActive: boolean) => {
    return `inline-flex items-center gap-1.5 rounded-md px-3 py-2 text-xs font-semibold transition-all duration-300 ${
      isActive
  ...
  const getMobileLinkClass = (isActive: boolean) => {
    return `block rounded-md px-3 py-2 text-sm font-medium transition-all ${
      isActive
        ? "bg-blue-500/20 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400"
        : "text-slate-600 hover:bg-slate-200/60 hover:text-slate-900 dark:text-slate-400 dark:hover:bg-white/5 dark:hover:text-white"
    }`;
  ...
  const renderMobileNavContent = (label: string, isActive: boolean) => (
    <>
      {isActive && <span className="w-1.5 h-1.5 bg-custom rounded-full animate-pulse" />}
      {label}
    </>
  );
